...
MC_NODISCARD auto operator==(Vector const& l, Vector const& r)
    -> std::enable_if_t<IsVector<Vector>::value, bool>
{
...
template <typename Vector>
MC_NODISCARD auto operator*(
    Vector const& vec, typename Vector::value_type const& scaler)
    -> std::enable_if_t<IsVector<Vector>::value, Vector>
{
...
template <typename Vector>
MC_NODISCARD auto operator*(typename Vector::value_type const& scaler,
    Vector const& vec) -> std::enable_if_t<IsVector<Vector>::value, Vector>
{
    return vec * scaler;
}

template <typename Vector>
auto operator<<(std::ostream& out, Vector const& vec)
    -> std::enable_if_t<IsVector<Vector>::value, std::ostream&>
{
