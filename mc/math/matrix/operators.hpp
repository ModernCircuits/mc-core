...
auto operator==(Matrix const& l, Matrix const& r)
    -> std::enable_if_t<IsMatrix<Matrix>::value, bool>
{
...
template <typename Matrix>
auto operator+(Matrix const& m, typename Matrix::value_type scaler)
    -> std::enable_if_t<IsMatrix<Matrix>::value, Matrix>
{
...
template <typename Matrix>
auto operator+(typename Matrix::value_type scaler, Matrix const& m)
    -> std::enable_if_t<IsMatrix<Matrix>::value, Matrix>
{
    return detail::matrixAdd(m, scaler);
}
...
auto operator*(Matrix const& mat, Vector const& vec)
    -> std::enable_if_t<IsMatrix<Matrix>::value && IsVector<Vector>::value,
        Vector>
{
...
template <typename Matrix>
auto operator<<(std::ostream& out, Matrix const& m)
    -> std::enable_if_t<IsMatrix<Matrix>::value, std::ostream&>
{
