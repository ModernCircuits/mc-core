...
.PHONY: coverage
coverage:
	conan install -of cmake-build-coverage --build=missing -pr=default -s compiler.cppstd=${CXX_STD} -s build_type=Debug .
	cmake -S . -B=cmake-build-coverage -DCMAKE_TOOLCHAIN_FILE="cmake-build-coverage/conan_toolchain.cmake" -DCMAKE_BUILD_TYPE=Debug -DCMAKE_CXX_STANDARD="${CXX_STD}" -DMODERNCIRCUITS_ENABLE_COVERAGE=TRUE
